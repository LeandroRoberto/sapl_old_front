...
function contextBgColor(href) {
	return;
	$('.context-bg-color').removeClass('container-materia').removeClass('container-autografo').removeClass('container-norma').removeClass('container-diario-oficial').removeClass('container-sessao');

	if (href.indexOf('/sessao_plenaria/') != -1) {
		colorThemeSapl('container-sessao');
		$('.navbar.main a[href*=sessao_plenaria]').addClass('active');
	}
    else if ($('#qs-tc').val() == 'tm')
		colorThemeSapl('container-materia');
	else if ($('#qs-tc').val() == 'tn') {
		colorThemeSapl('container-norma');
	}
	else if ($('#qs-tc').val() == 'ta') {
		colorThemeSapl('container-autografo');
	}
	else if ($('#qs-tc').val() == 'td') {
  ...
  //var link = "http://sapl.camarajatai.go.gov.br/sapl/qs"
    ...
    qs.keyup(function(event) {
        if (!event.ctrlKey && event.keyCode == 13) {
			window.history.pushState(formData, "sapl", link+'?q='+$('#qs').val());
			goTop();

		    contextBgColor(link+'?q='+$('#qs').val());
			jQuery( "#qs-p" ).val(1)
			autoscroll = false;
            qSearch(qs.value)
